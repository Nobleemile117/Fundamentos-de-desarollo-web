...
<head>
  ...
  <title>Cortes – Panel Admin</title>
...
<body>
  ...
  <div class="admin-wrap">
    <h1 class="admin-page-title">Cortes</h1>
    <p class="admin-page-sub">Agregar, editar y eliminar cortes del menú · <span style="color:#c8b89a;">* campos obligatorios</span></p>

    <!-- FORM: AGREGAR CORTE -->
    <form class="admin-form" action="../db/insertFood.php" method="POST" enctype="multipart/form-data" onsubmit="return validateForm()">
      <h2>Agregar Corte</h2>
    ...
    <!-- LISTA DE CORTES -->
    <h2 class="admin-page-title" style="font-size:1.4rem; margin-bottom:20px;">Cortes registrados</h2>
          ...
          <div class="item-info">
            ...
            <div class="item-badges">
              ...
              <?php if($item['show_price']) { ?>
                <span class="badge badge-price">Precio visible</span>
              <?php } else { ?>
                <span class="badge badge-noprice">Precio oculto</span>
              <?php } ?>
            </div>
          </div>
          <div class="item-actions">
            <button class="btn-edit"   onclick="window.location.href='editFood.php?id=<?php echo $item['id']; ?>'">Editar</button>
            <button class="btn-delete" onclick="confirmDelete('../db/deleteFood.php?id=<?php echo $item['id']; ?>')">Eliminar</button>
          </div>
        </div>
      <?php } ?>
    </div>
