<?php
    ...
    if(!isset($_GET['id'])) { header("Location: menu-desserts.php?error=Id inválido"); return; }

    $id     = $_GET['id'];
    $result = mysqli_query($conn, "SELECT * FROM desserts WHERE id = $id");
    $item   = mysqli_fetch_array($result);

    if(!$item) { header("Location: menu-desserts.php?error=Postre no encontrado"); return; }
?>
<!DOCTYPE html>
<html lang="es">
<head>
  ...
  <title>Editar Postre – Panel Admin</title>
...
<body>
  ...
  <div class="admin-wrap">
    <h1 class="admin-page-title">Editar Postre</h1>
    <p class="admin-page-sub">Modifica los datos del postre seleccionado</p>

    <form class="admin-form" action="../db/updateDessert.php" method="POST" enctype="multipart/form-data">
      ...
      <?php if($item['image']) { ?>
        <div>
          ...
          <img src="../assets/images/uploads/desserts/<?php echo $item['image']; ?>"
               alt="Imagen actual"
